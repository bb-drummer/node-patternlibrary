...
module.exports = function(file, opts, cb) {
    ...
    //this.$PL.debug('page: ', page);
    //this.$PL.debug('adapters: ', this.adapters);
    ...
    async.parallel(parseTasks, function(err, results) {
        ...
        //_this.$PL.debug('tree: ', _this.tree);
        cb(null, page);
    });
}
...
function findPath(fileGlobs, collider) {
	//collider.$PL.debug('path: ', path.join(collider.filePath, fileGlobs));
...
function addFiles(files, fileGlobs, collider, cb) {
	var searchGlob = fileGlobs; //findPath( fileGlobs, collider );
