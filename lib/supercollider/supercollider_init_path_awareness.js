    ...
    function transform() {
        return through.obj(function(file, enc, cb) {
    	    ...
    	    this.parse(file, opts, function(err, data) {
	        	...
	        	var docfile = String(file.path)
    	            .replace(file.base , ''); // kill /base/path/to/src/.../
	        	var pattern = String(docfile)
	        	    .replace(path.basename(file.path) , ''); // kill filename.ext

	            var pagedata = this.build(data)
	            
    		    file.contents = new Buffer( pagedata );

		        // Write new file to disk if necessary
		        if (this.options.dest) {
	                var filePath = path.join(this.options.dest, docfile);
		        	if (this.options.targetfilename && (this.options.targetfilename != '')) {
		        		filePath = path.join(
		        			this.options.dest, 
		        			pattern, 
		        			this.options.targetfilename
		        		);
		        	}
