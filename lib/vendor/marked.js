...
mdRenderer.code = function(code, language) {
  ...
  // render anny patern-data
  //$PL.debug('code before: ', code);
  code = $PL.getDocRenderer().renderPattern(code, $PL.Panini.data);
  //$PL.debug('code after: ', code);
...
module.exports = function ( patternlibrary ) {
	mdRenderer.patternlibrary = $PL = patternlibrary;
	return mdRenderer;
}
