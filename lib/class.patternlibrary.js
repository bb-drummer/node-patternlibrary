...
class Patternlibrary {
	...
	/**
	 * process (missing) defaults and parameters
	 * 
	 * @param object data
	 * @return object
	 */
	...
	updateDataFile ( ) { 
		...
		var obj  = this.getPatterns();
			 
		jsonfile.writeFileSync(file, obj, {spaces: 4, flag: 'w'});
	}
	...
	refresh ( ) { 
		...
		var partialsPath = (this.options.panini.partials || 'partials');
		// gather partials
	...
	_registerPatternHelpers ( ) {
		this.Panini.loadLayouts(['node_modules/patternlibrary/layouts','layouts',
			(this.options.panini.layouts || 'layouts')
		]);
		this.Panini.loadHelpers(['node_modules/patternlibrary/helpers','helpers',
			(this.options.panini.helpers || 'helpers')
		]);
	}
